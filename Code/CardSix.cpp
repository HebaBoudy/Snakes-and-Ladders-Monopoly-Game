...
void CardSix::Save(ofstream& OutFile, int objecttype)
{
	if (objecttype == 3) {
		OutFile << cardNumber << " " << position.GetCellNum() << " " << CellToMove.GetCellNum() << endl;
	}
}
void CardSix::Load(ifstream& Infile)
{
	int  cellposition, destination;
	Infile >> cellposition >> destination;
	CellPosition Loadpos(cellposition);
	CellPosition LoadDestination(destination);
	position = Loadpos;
	CellToMove = LoadDestination;
}
