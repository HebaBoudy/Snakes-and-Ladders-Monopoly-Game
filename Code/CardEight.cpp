...
void CardEight::Save(ofstream& OutFile, int objecttype)
{
	if (objecttype == 3) {
		OutFile << cardNumber << " " << position.GetCellNum() << " " << AmountOfCoins << endl;
	}
}
void CardEight::Load(ifstream& Infile)
{
	int  cellposition, prisoncoins;
	Infile >> cellposition >> prisoncoins;
	CellPosition LoadPos(cellposition);
	AmountOfCoins = prisoncoins;
	position = LoadPos;
}
