...
CellPosition Grid::setall9(Player* Owner) {
	
	for (int i = NumVerticalCells - 1; i >= 0; i--) {
		for (int j = 0; j < NumHorizontalCells; j++) 
		{
			CardNine* cardnine = dynamic_cast<CardNine*>(CellList[i][j]->GetGameObject());
			if (cardnine != NULL) {
				return Owner->GetCell()->GetCellPosition();
			}
...
void Grid::SaveAll(ofstream& savefile, int objecttype) {
	if (objecttype == 1) {
		for (int i = NumVerticalCells - 1; i >= 0; i--)
		{
			for (int j = 0; j < NumHorizontalCells; j++)
